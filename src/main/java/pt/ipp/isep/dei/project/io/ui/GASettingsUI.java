...
class GASettingsUI {
        ...
        while (activeInput) {
            ...
            switch (option) {
                ...
                case 0:
                    return;
                default:
                    System.out.println(utilis.invalidOption);
                    break;
            }
        }
    }
    ...
    private TypeArea getInputTypeAreaByList(TypeAreaList typeAreaList) {
        UtilsUI utilis = new UtilsUI();
        InputUtils inputUtils = new InputUtils();
        if (!(utilis.typeAreaIsValid(typeAreaList))) {
 ...
 classification of geographical areas.*/
    ...
    private void updateAndDisplayUS02(TypeAreaList typeAreaList) {
        ...
        if (utilis.typeAreaIsValid(typeAreaList)) {
            ...
            System.out.println(utilis.invalidGATypeList);
        }
    }
    ...
    private void getAreaInputUS03(GeographicAreaList geographicAreaList, TypeAreaList typeAreaList) {
        UtilsUI utilis = new UtilsUI();
        if ((geographicAreaList != null) && (utilis.typeAreaIsValid(typeAreaList))) {
            ...
            System.out.println("The type area list is invalid or the geographic area list is invalid. Please fix before proceeding.");
        }
    }
    ...
    private void displayGAListByTypeArea(GeographicAreaList geographicAreaList) {
        if (this.mGeoAreaList == null || this.mTypeArea == null || (!(utilis.geographicAreaListIsValid(geographicAreaList)))){
            return;
        }
    ...
    private void getInputMotherDaughterGA(GeographicAreaList programGAList) {
        UtilsUI utilis = new UtilsUI();
        if (utilis.geographicAreaListIsValid(programGAList)) {
            ...
            System.out.println(utilis.invalidGAList);
        }
    }

    private void updateStateUS07(GeographicAreaList newGeoListUi) {
        UtilsUI utilis = new UtilsUI();
        if (utilis.geographicAreaListIsValid(newGeoListUi) && mMotherAreaName != null && mDaughterAreaName != null) {
    ...
    private void getContainerArea(GeographicAreaList geographicAreaList) {
        UtilsUI utilis = new UtilsUI();
        if (utilis.geographicAreaListIsValid(geographicAreaList)) {
    ...
    private void getContainedArea(GeographicAreaList geographicAreaList) {
        UtilsUI utilis = new UtilsUI();
        if ((utilis.geographicAreaListIsValid(geographicAreaList)) && (mGeoArea != null)) {
    ...
    private void checkIfContained(GeographicAreaList geographicAreaList) {
        UtilsUI utilis = new UtilsUI();
        if ((utilis.geographicAreaListIsValid(geographicAreaList)) && (mContainedAreaName != null) && (mContainerAreaName != null)) {
